...
class Logger(object):
    def __init__(self):
        ...
        self.log = open("output_minimax.txt", "wt")
...
# Compute optimal action based on minimax at a state

def minimax(state, player, depth=0):

    uval = utility(state)
    idx = player-1

    if is_end(state) or depth==minmax_depth:
        return (uval, None)

    v = player * -math.inf
    next_action = None

    # if player == 4 or player == 3 or player == 2 or player == 1:
    x, y = get_pos(state, player)
    actions = available_moves(state, x, y)
    for action in actions:
        new_state = move(state, player, action)
        eval = minimax(new_state, player, depth+1)
        tmp = eval[0][0]
        if v < tmp:
            v = tmp
            next_action = action
...
def print_node(current_player, father_node, action, current_node):
    print("[Current_player={} | Father_node={} | Action={} | Current_node={} | {} | MINIMAX={}]".format(
        current_player, father_node, action, current_node, winner(current_node), minimax(current_node, current_player))
    )
